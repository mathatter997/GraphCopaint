...
for ((i = 0; i < ${#opt_num_paths[@]}; i++)); do
    CUDA_VISIBLE_DEVICES="0" python inference.py --config_type ego_small  --cpu False --num_samples 32 \
        ...
        --lr_xt_path ${lr_xt_path} --opt_num_path ${opt_num_paths[$i]}.json \
        ...
        --output_path data/dataset/ablation/ego/output_copaint_${opt_num_paths[$i]}.json \
        --mask_path data/dataset/ablation/ego/mask_copaint_${opt_num_paths[$i]}.json \
        --masked_output_path data/dataset/ablation/ego/masked_output_copaint_${opt_num_paths[$i]}.json \
