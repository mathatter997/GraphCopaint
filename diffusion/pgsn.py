...
class PGSN(nn.Module):
    ...
    def __init__(self, nf=64, num_gnn_layers=4, dropout=0.1, embedding_type='positional', rw_depth=24, edge_th=0.2, act=nn.SiLU(),
                 heads=4, max_node=None, graph_layer='PosTransLayer', attn_clamp=False):
        super().__init__()
        ...
        # degree onehot
        self.degree_max = self.max_node // 2
        ...
        # GNN
        modules.append(gnns.pos_gnn(act, self.x_ch, self.pos_ch, nf, self.max_node,
                                    self.graph_layer, num_gnn_layers,
                                    heads=self.heads, edge_dim=nf//2, temb_dim=nf * 4,
                                    dropout=dropout, attn_clamp=self.attn_clamp))
