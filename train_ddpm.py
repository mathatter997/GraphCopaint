...
@click.option('--mixed_precision', default='no', type=click.Choice(['yes', 'no'], case_sensitive=False))
@click.option('--cpu', default=False)
@click.option('--data_filepath')
@click.option('--data_name')
def train_ddpm(checkpoint_path, mixed_precision, cpu, data_filepath, data_name):
    @dataclass
    ...
    dataloader = DataLoader(train_dataset, batch_size=config.train_batch_size, shuffle=True) # load lobsters
    ...
    noise_scheduler = DDPMScheduler(num_train_timesteps=config.train_timesteps, 
